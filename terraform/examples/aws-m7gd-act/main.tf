...
# AWS Standard ACT
#
# Run standard ACT test parameters against m7gd instance types to validate ACT rating.
#
...
variable "my_aws_ami_name" {
  description = "Use aerospike-act-arm64-6* for m7gd Graviton ARM64 instances"
  type        = string
...
module "act_m7gd" {
  ...
  s3_path   = "aws-m7gd-act/${formatdate("YYYY-MM-DD-hh-mm-ss", timestamp())}"
...
output "ssh_logins" {
  value = [for m in module.act_m7gd : m.ssh_logins]
}
